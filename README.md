...
在已 root 的 Android ARM64 设备上，从 ART 运行时抓取真实 DEX，把执行过的方法字节码回填到 dump 文件；也支持 native `.so` dump 和 JNI 动态注册名恢复。仅用于授权的 Android 逆向与安全研究。
...
仓库提供 [Android DEX dump Skill](skills/android-dex-dump/SKILL.md)，可一键完成本地 APK 的安装、启动、dump、拉取与校验：

```bash
./skills/android-dex-dump/scripts/run_dump.sh /path/to/app.apk
```

脚本按 `full → lifecycle → maps-only` 依次尝试，结果和 `report.txt` 写入 Downloads 下的新目录。仅用于已授权的 APK 和设备。
